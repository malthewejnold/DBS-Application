<template>
	<v-container>
		...
		<v-row>
			<v-col>
				<v-btn class="purple lighten-3 mb-4 white--text" dark @click="createApplicant">Opret ansøger</v-btn>
			</v-col>
		</v-row>
	</v-container>
</template>
...
export default {
	...
	methods: {
		//Creates the applicant in the database
		async createApplicant() {
			this.applicant.initials = this.$store.state.user

			try {
				await ApplicantsService.createApplicant(this.applicant);
			} catch (err) {
